...
public class MailEditActivity extends Activity implements OnClickListener {
		...
		private Handler handler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				case SUCCESS:
					...
					if(mailid>0){
						Uri uri=Uri.parse("content://com.caogaoxiangprovider1");
						getContentResolver().delete(uri, "id=?", new String[]{mailid+""});
						uri=Uri.parse("content://com.attachmentprovider1");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,R.layout.left_mail_send_titlebar);
	...
	public void back(View v){
		if(isCaogaoxiang&&!mail_to.getText().toString().equals("")){
	...
	private void saveToCaogaoxiang(){
		...
		if(adapter.getList().size()>0){
			Uri att_uri=Uri.parse("content://com.attachmentprovider1");
